...
applied a configured ``--confidence-floor``; examples contradicted the
environment context shipped alongside them, telling the model that signedness
could not be known when the config stated it and that a cast to long truncates
under LP64; and the function-first prompt's JSON examples reached the model with
doubled braces, so every example of the required output format was malformed.
...
def _legacy_prompts(client: LLMClient) -> dict[str, str]:
...
def test_function_prompt_quotes_the_configured_floor(floor: float) -> None:
    ...
    assert f"{floor:.2f}".rstrip("0").rstrip(".") in prompt
...
def test_function_prompt_quotes_the_default_floor() -> None:
    ...
    assert str(DEFAULT_CONFIDENCE_FLOOR) in prompt
...
def test_legacy_prompts_quote_the_configured_floor(name: str) -> None:
    ...
    for pass_name, prompt in _legacy_prompts(client).items():
        assert "0.85" not in prompt, f"{pass_name} still quotes a hardcoded floor"


def test_environment_context_quotes_the_configured_floor() -> None:
    context = LLMClient("none", "none", ILP32_SIGNED, confidence_floor=0.6)._build_environment_context()

    assert "0.6" in context
    assert "0.85" not in context
...
# --- Examples agree with the environment context they ship with ---------------
...
def test_lp64_examples_do_not_claim_a_cast_to_long_truncates() -> None:
    examples = LLMClient("none", "none", LP64)._get_lp64_examples()

    long_cast = next(e for e in examples if "(long)" in e["code"])
    assert long_cast["response"]["y2038_issue"] == "no"
    assert "truncate" not in long_cast["response"]["reason"].lower()


def test_lp64_examples_still_teach_the_narrowing_case() -> None:
    examples = LLMClient("none", "none", LP64)._get_lp64_examples()

    narrowing = [e for e in examples if "int32_t" in e["code"]]
    assert narrowing, "LP64 needs an example of the one pattern that is a risk there"
...
def test_a_stored_time_value_is_judged_by_the_environment(name: str, expected: str) -> None:
    verdict, _, reason = LLMClient("none", "none", ENVIRONMENTS[name])._stored_time_verdict()

    assert verdict == expected
    if name == "absent":
        assert "no environment configuration" in reason


@pytest.mark.parametrize("name", ["ilp32_signed", "ilp32_unsigned", "lp64"])
def test_every_pass_answers_a_stored_time_value_the_same_way(name: str) -> None:
    """Pass 2 and pass 3 examples once asserted 32-bit signed whatever the target."""
    client = LLMClient("none", "none", ENVIRONMENTS[name])
    expected = client._stored_time_verdict()[0]

    for examples in (client._get_pass2_examples(), client._get_pass3_examples()):
        stored = [
            e["response"]
            for e in examples
            if "time(NULL)" in str(e.get("context", "") or e.get("file", ""))
            or "tv_sec" in str(e.get("context", "") or e.get("file", ""))
        ]
        assert stored, "expected a stored-time example in every pass"
        assert all(response["y2038_issue"] == expected for response in stored)


def test_no_example_set_repeats_itself() -> None:
    """The ILP32 signed set listed the same negative check twice, under two ids."""
    for name in ENVIRONMENTS:
        client = LLMClient("none", "none", ENVIRONMENTS[name])
        codes = [example["code"] for example in client._get_scenario_examples()]
        assert len(codes) == len(set(codes)), f"{name} repeats an example"
...
def test_function_prompt_examples_are_valid_json(detect_y2106: bool) -> None:
    ...
    examples = re.findall(r"^Response: (\{.*\})$", prompt, flags=re.MULTILINE)
...
def test_every_pass_shows_the_function_body_as_written(tmp_path: Path) -> None:
    ...
    with_file_context = client._build_pass_f3_prompt(batch)
    assert "struct holder { time_t at; };" in with_file_context
