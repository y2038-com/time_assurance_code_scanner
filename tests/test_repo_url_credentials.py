...
A URL such as ``https://user:TOKEN@github.com/org/private.git`` would otherwise
...
CREDENTIAL_URLS = [
    f"https://user:{TOKEN}@github.com/org/private.git",
    f"https://{TOKEN}@github.com/org/private.git",
    f"http://user:{TOKEN}@example.com/org/private.git",
    f"ssh://user:{TOKEN}@example.com/org/private.git",
]
...
def test_redaction_keeps_host_and_path_readable() -> None:
    assert (
        bsr.redact_url_credentials(f"https://user:{TOKEN}@github.com/org/p.git")
        == "https://***@github.com/org/p.git"
    )
    assert (
        bsr.redact_url_credentials(f"ssh://user:{TOKEN}@example.com/org/p.git")
...
def test_redaction_works_on_surrounding_text() -> None:
    ...
    message = (
        f"git clone --no-tags https://user:{TOKEN}@github.com/org/p.git /tmp/x failed: "
        f"fatal: could not read from https://user:{TOKEN}@github.com/org/p.git"
    )
...
def test_rejection_warning_points_at_the_alternatives(tmp_path: Path) -> None:
    repos_file = tmp_path / "repos.jsonl"
    repos_file.write_text(
        json.dumps({"repo_url": f"https://{TOKEN}@github.com/org/p.git"}) + "\n",
        encoding="utf-8",
    )
...
def test_run_artifacts_and_console_never_carry_the_token(
    ...
    repos_file = tmp_path / "repos.jsonl"
    repos_file.write_text(
        json.dumps({"repo_url": f"https://user:{TOKEN}@github.com/org/private.git"})
        + "\n",
        encoding="utf-8",
    )
    ...
    with pytest.raises(RuntimeError) as excinfo:
        bsr._git(
            [
                "clone",
                "--no-tags",
                f"https://user:{TOKEN}@127.0.0.1:1/org/p.git",
                str(tmp_path / "dest"),
            ],
            timeout=60,
        )
